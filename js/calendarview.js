...
function handlePopupUI(calendar, style) {
  ...
  if (title && nextYear && nextMonth && prevYear && prevMonth) {
    ...
    var calendarNewYear = newHeader.querySelector('.calendar-new-year[aria-live="polite"]');
      ...
      newHeader.innerHTML = '<div class="calendar-new-month" aria-live="polite"><span class="calendar-new-month-text">'+month+'</span></div><div class="calendar-new-year" aria-live="polite"><span class="calendar-new-year-text">'+year+'</span></div>';
...
Calendar.prototype = {
      ...
      function(row, i) {
        ...
        row.immediateDescendants().each(
          function(cell, j) {
            ...
            cell.setAttribute('aria-label', cell.date.toLocaleDateString('en-US', { day: 'numeric', weekday: 'long' }));
  ...
  create: function(parent, id)
  {
    ...
    for (var i = startDay; i <= endDay; ++i) {
      ...
      cell = new Element('th', { 'aria-hidden': true }).update(Calendar.SHORT_DAY_NAMES[i]);
